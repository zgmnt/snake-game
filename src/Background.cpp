...
{	//  background alpha depends on background_alpha_clock  //
	...
	if (background_alpha_clock.getElapsedTime().asSeconds() < 2.54)
		sprite_current_background.setColor(sf::Color(255, 255, 255,
			background_alpha_clock.getElapsedTime().asSeconds() * 100));
	else if ((background_alpha_clock.getElapsedTime().asSeconds() < 30) 
		&& (background_alpha_clock.getElapsedTime().asSeconds() > 27.45))
		sprite_current_background.setColor(sf::Color(255, 255, 255, 510 
			- ((background_alpha_clock.getElapsedTime().asSeconds() - 24.9) * 100)));
	else
		sprite_current_background.setColor(sf::Color(255, 255, 255, 255));
}
